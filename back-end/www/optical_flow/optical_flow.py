...
class OpticalFlow(object):
    ...
    def vid_to_frames(self):
        ...
        height = 224 # np.size(previous_frame, 0)
        width = 224 # np.size(previous_frame, 1)
        length = int(capture.get(cv.CAP_PROP_FRAME_COUNT))
        rgb_4d = np.zeros((length, height, width, 3), dtype=np.uint8)

        if length <= 1:
            return None

        for i in range(length):
            rgb_4d[i, :, :, :] = cv.resize(cv.cvtColor(previous_frame, cv.COLOR_BGR2RGB), (width, height))
